...
const roleKeyMap: Record<string, keyof typeof import("@/i18n/en").default.data.roles> = {
  President: "president",
  "Vice President": "vicePresident",
  Secretary: "secretary",
  Treasurer: "treasurer",
  "Cultural Secretary": "culturalSecretary",
  "Membership Chair": "membershipChair",
  "Events Coordinator": "eventsCoordinator",
      ...
      <section className="gradient-saffron py-16 md:py-20">
        <div className="container">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-4xl font-bold text-primary-foreground font-display md:text-5xl">{t.leadership.title}</h1>
            <p className="mt-2 text-primary-foreground/80">{t.leadership.subtitle}</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container">
          <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {leadershipTeam.map((member, i) => {
              const roleKey = roleKeyMap[member.title];
              return (
                <motion.div
                  key={member.id}
                  ...
                  <Card className="overflow-hidden text-center hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-4">
                      ...
                      <p className="text-sm text-primary font-medium">
                        {roleKey ? t.data.roles[roleKey] : member.title}
                      </p>
                    </CardContent>
                  </Card>
                </motion.div>
